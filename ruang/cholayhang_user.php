            ...
            <li class="nav-item active">
                ...
                <div id="collapseForm" class="collapse" aria-labelledby="headingForm" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item active" href="./cholayhang_user.php">Chờ lấy hàng</a>
                                    ...
                                    <tbody>
                                        <?php
                                            ...
                                            //thêm where makh (đăng nhập xong) hoac mashipper (KH voi shipper dung chung trang nay)
                                                $sql="SELECT * from donhang JOIN user ON donhang.makh=user.ma WHERE makh = '$ma' AND  (trangthai='1' OR trangthai='2')";
                                                $query = mysqli_query($conn,$sql);	
                                                $row = array();
                                                while($data = mysqli_fetch_assoc($query)){
                                                    $row[] = array($data['madh'],$data['tensp'],$data['tienthuho'],$data['tenNN'],$data['sdtNN'],$data['diachiNN'],$data['img'],$data['goicuoc'],$data['tuychon'],$data['trangthai'],$data['ghichu'],$data['hinhthuc']);
                                                }
                                        ...
                                        <form action="./updateTrangThai.php" method="post">
                                            <tr>
                                                ...
                                                <td><?php echo $row[$j][11]; ?></td> 
                                                <td><?php echo $row[$j][10]; ?></td>       
                                                <!-- neu la shipper thi sua lai la Da nhan hang -->
                                                <td> <input type="submit" value="Đã gửi hàng" name="guihang1" class="btn btn-warning mb-1"></td>
                                                <input type="hidden" name="id" value="<?php echo $row[$j][0]; ?>">
                                                <input type="hidden" name="trangthai" value="<?php echo $row[$j][9]; ?>">
                                            </tr>
                                        </form>
                                        <?php } ?>                                        
                                    </tbody>
